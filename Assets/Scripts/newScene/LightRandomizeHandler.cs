...
public class LightRandomizeHandler : RandomizerInterface
{
    ...
    private int nextRandomLightIdx;

    public void Start()
    {
        this.nextRandomLightIdx = 0;
        
        this.LinkGui("LightRandomizerList");
    ...
    private void RandomizeExtraLights(ref RandomNumberGenerator rng)
    {
        ...
        for (int i = 0; i < dataset.numLightsources; ++i)
        {
            ...
            float radius = Utils.convertMmToUnity(rng.Range(dataset.minRadiusLight, dataset.maxRadiusLight));
    ...
    private void RandomizeEnvironment(ref RandomNumberGenerator rng)
    {
        ...
        if (cubeMaps.Length > 0)
        {
            ...
            {
                texture = (Cubemap)cubeMaps[this.nextRandomLightIdx];
                this.nextRandomLightIdx = (this.nextRandomLightIdx + 1) % cubeMaps.Length;
            }
            else
    ...
    public override bool updateCheck(uint currentUpdate, MainRandomizerData.RandomizerUpdateIntervals[] updateIntervals = null)
    {
        ...
        {
            if (pair.randomizerType == MainRandomizerData.RandomizerTypes.Light)
            {
