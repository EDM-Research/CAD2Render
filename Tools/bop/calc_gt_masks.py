...
def render_bop_masks(datasets_path, dataset, dataset_split, dataset_split_type, renderer_type, obj_ids, scene_ids,im_size, delta):
    base_path = join(datasets_path, dataset)
    # Path to the split directory.
    split_path = join(base_path, dataset_split)
    if dataset_split_type is not None:
        split_path += '_' + dataset_split_type
    ...
    for scene_id in scene_ids:
      ...
      # Load scene GT.
      scene_gt_path = scene_gt_tpath.format(
        scene_id=scene_id)
      scene_gt = inout.load_scene_gt(scene_gt_path)

      # Load scene camera.
      scene_camera_path = scene_camera_tpath.format(
        scene_id=scene_id)
      ...
      ren = renderer.create_renderer(
        width, height, renderer_type=renderer_type, mode='depth')
      ...
      models_path = join(datasets_path, dataset, models_folder_name)
      model_tpath = join(models_path, 'obj_{obj_id:06d}.ply')
      for obj_id in obj_ids:
        ren.add_object(obj_id, model_tpath.format(obj_id=obj_id))
        
      im_ids = sorted(scene_gt.keys())
      
      for im_id in im_ids:
        if im_id % 99:
            print("\rCalculating masks %06d/%06d"%(im_id+1,len(scene_gt)) , end =" ")
        # if im_id % 100 == 0:
          # misc.log(
            # 'Calculating masks - dataset: {} ({}, {}), scene: {}, im: {}'.format(
              # dataset, dataset_split, dataset_split_type, scene_id,
              # im_id))
...
def main():
    ...
    ################################################################################
    render_bop_masks(datasets_path, dataset, dataset_split, dataset_split_type, renderer_type, p['delta'])
